...
void main() async { //메인함수에 사용가능하대
  WidgetsFlutterBinding.ensureInitialized();  //이코드는 플러터프레임워크가 준비됐는지 확인하는 코드, 원래는 런앱할떄 자동으로 됨
  
  await initializeDateFormatting();   //intl패키지를 런앱하기전에 초기화해줌

  final database = LocalDatabase(); //데이터 베이스 생성

  GetIt.I.registerSingleton<LocalDatabase>(database); //GetIT를 사용하여 어디서든 (dateabase)값을 가져올수 있음 따로 파라미터를 안들고와도 상관없음
